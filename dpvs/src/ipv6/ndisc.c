...
void ndisc_send_rs(struct netif_port *dev, const struct in6_addr *solicit, const struct in6_addr *daddr, const struct in6_addr *saddr)
{
    struct rte_mbuf *mbuf;
    struct icmp6_hdr icmp6h;
    struct flow6 fl6;
    //int err;
    memset(&icmp6h, 0, sizeof(icmp6h));
    icmp6h.icmp6_type = ND_ROUTER_SOLICIT;
    mbuf = ndisc_build_mbuf(dev, daddr, saddr, &icmp6h, solicit, 0);
    if (!mbuf)
        return;
    memset(&fl6, 0, sizeof(fl6));
    fl6.fl6_oif   = dev;
...
static int ndisc_router_discovery(struct rte_mbuf *mbuf, struct netif_port *dev)
{

    uint8_t *lladdr = NULL;
    struct ra_msg *ra_msg = rte_pktmbuf_mtod(mbuf, struct ra_msg *);
    ...
    ndoptlen = mbuf->data_len - sizeof(struct ra_msg);

    char cbuf[64];

    inet_ntop(AF_INET6, &iph->ip6_src, cbuf, sizeof(cbuf));
    ...
    inet_ntop(AF_INET6, &iph->ip6_src, sbuf, sizeof(sbuf));
    inet_ntop(AF_INET6, &iph->ip6_dst, dbuf, sizeof(dbuf));

    RTE_LOG(DEBUG, IPV6, "%s: [%d], %s -> %s\n",
            __func__, rte_lcore_id(), sbuf, dbuf);
    

    if (mbuf_may_pull(mbuf, sizeof(struct ra_msg))) {

        RTE_LOG(ERR, NEIGHBOUR, "ICMPv6 RA: packet too short.\n");
        return EDPVS_DROP;
    } 
    
    if (!(ipv6_addr_type(&iph->ip6_src) & IPV6_ADDR_LINKLOCAL)) {
		RTE_LOG(ERR, NEIGHBOUR, "RA: source address is not link-local, RA src %s -> dst %s\n",sbuf, dbuf );
		return EDPVS_DROP;
	}
 

    if (!ndisc_parse_options(opt, ndoptlen, &ndopts)) {
        RTE_LOG(ERR, NEIGHBOUR, "[%s] NS: invalid ND packet\n", __func__);
        return EDPVS_DROP;
    }
    ...
    if (ndopts.nd_opts_src_lladdr) {
        lladdr = ndisc_opt_addr_data(ndopts.nd_opts_src_lladdr, dev);
        if (!lladdr) {
            RTE_LOG(ERR, NEIGHBOUR, " NA: invalid link-layer address\n", __func__);
            return EDPVS_DROP;
        }
    }
    ...
    int i;
        for(i=0;i<16;i++)
	    {
		        anyaddr.s6_addr[i]=00;
          
	    }
 
   
    slaac_add_del(&anyaddr, 0, 1024, saddr,  dev, &anyaddr, 0, true);
    neigh = neigh_lookup_entry(AF_INET6, (union inet_addr *)saddr, dev, hashkey);
    if (neigh && !(neigh->flag & NEIGHBOUR_STATIC)) {
        neigh_edit(neigh, (struct ether_addr *)lladdr);
        neigh_entry_state_trans(neigh, 1);
    ...
    for (p=ndopts.nd_opts_pi;
		     ...
		     addrconf_prefix_rcv(dev, (uint8_t *)p,
					    (p->nd_opt_len) << 3,
...
int ndisc_rcv(struct rte_mbuf *mbuf, struct netif_port *dev)
{
    ...
    inet_ntop(AF_INET6, &ipv6_hdr->ip6_src, sbuf, sizeof(sbuf));
    inet_ntop(AF_INET6, &ipv6_hdr->ip6_dst, dbuf, sizeof(dbuf));
    ...
    case ND_ROUTER_ADVERT:
        ret = ndisc_router_discovery(mbuf, dev);
        break;
