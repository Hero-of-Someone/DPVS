...
static inline void addrconf_addr_eui48_base(__u8 *eui, const char *const addr);
static inline void addrconf_ifid_eui48(__u8 *eui, struct netif_port *dev);
static int ipv6_generate_stable_address(struct in6_addr *address, __u8 dad_count, const struct netif_port *dev);
int addrconf_prefix_rcv_add_addr(struct netif_port *dev,  const struct prefix_info *pinfo, 
                                 const union inet_addr *addr, __u32 valid_lft,
                                  __u32 prefered_lft);
int addrconf_add_linklocal(struct netif_port *idev,
...
static inline void addrconf_ifid_eui48(__u8 *eui, struct netif_port *dev)
{
    ...
    int i;
    for(i=0; i<6; i++)
        macadd[i]=dev->addr.addr_bytes[i];
    addrconf_addr_eui48_base(eui, macadd);
            if (dev->id) {
		           eui[3] = (dev->id >> 8) & 0xFF;
		           eui[4] = dev->id & 0xFF;
	        } else {
                                  ...
                                  __u32 prefered_lft)
{
		...
		int   af = AF_INET6;
        uint8_t             scope=0;
        uint32_t            flags=IFA_F_PERMANENT;    
        union inet_addr   bcast;
        int i;
        valid_lft=0;
        ...
        for(i=0;i<16;i++)
	    {
		   bcast.in6.s6_addr[i]=00;
	    }
        ifa->bcast=bcast;
        char bcastw[64];
        inet_ntop(AF_INET6, &ifa->bcast, bcastw, sizeof(bcastw));
        memcpy(&ifa->addr, addr, 16);
        char bcastw2[64];
        inet_ntop(AF_INET6, &ifa->addr, bcastw2, sizeof(bcastw2));

        const  struct ifaddr_action   slaac={
                .af=af,
                .addr=ifa->addr,
                .bcast = ifa->bcast,
                .plen = plen,
...
static int ipv6_generate_stable_address(struct in6_addr *address, __u8 dad_count, const struct netif_port *dev)
{
    ...
    static __u32 digest[5];
    static __u32 workspace[16];
    ...
    for(i=0;i<16;i++)
    {
    /*secret.s6_addr[i]= (uint32_t) random();*/
    secret.s6_addr[i]= (uint32_t) random_xid();
    }
    char addr2[64];
    inet_ntop(AF_INET6, (union inet_addr*)address, addr2, sizeof(addr2));
	sha1_init(digest);
	memset(&data, 0, sizeof(data));
	memset(workspace, 0, sizeof(workspace));
	memcpy(data.hwaddr, &dev->addr, dev->hw_header_len);
	data.prefix[0] = address->s6_addr32[0];
	data.prefix[1] = address->s6_addr32[1];
	data.secret = secret;
	data.dad_count = dad_count;
    char mac[64];
    inet_ntop(AF_INET6, (union inet_addr*)data.hwaddr, mac, sizeof(mac));
	sha1_transform(digest, data.__data, workspace);
    temp = *address;
	temp.s6_addr32[2] =  (__be32 )digest[0];
	temp.s6_addr32[3] =  (__be32 )digest[1];

    *address = temp;
    
    char addr3[64];
    inet_ntop(AF_INET6, (union inet_addr*)(address), addr3, sizeof(addr3));
...
void addrconf_prefix_rcv(struct netif_port *dev, uint8_t *opt, int len, bool sllao)
{
	...
	__u32 prefered_lft;
    int addr_type;
    struct inet_device *in6_dev;

    if (len < sizeof(struct prefix_info)) {
		RTE_LOG(DEBUG, IFA,"addrconf: prefix option too short\n");
		return;
    }
    ...
    inet_ntop(AF_INET6, (union inet_addr*)&(pinfo->prefix), pre, sizeof(pre));

    valid_lft = ntohl(pinfo->valid);
	prefered_lft = ntohl(pinfo->prefered);

    if (prefered_lft > valid_lft) {
		RTE_LOG(DEBUG, IFA,"addrconf: prefix option has invalid lifetime\n");
		return;
	}

    in6_dev = dev_get_idev(dev);

	if (!in6_dev) {
		RTE_LOG(DEBUG, IFA, "addrconf: device %s not configured\n",dev->name);
		return;
	}
     ...
     if (pinfo->autoconf ) 
    {
        ...
        if (pinfo->prefix_len == 64) {
            ...
            char addr2[64];
            inet_ntop(AF_INET6, (union inet_addr*)&(addr), addr2, sizeof(addr2));

            ipv6_generate_stable_address(&addr, 0, dev);
            addrconf_ifid_eui48(addr.s6_addr+8, dev);           

            char addr3[64];
            inet_ntop(AF_INET6, (union inet_addr*)&(addr), addr3, sizeof(addr3));
...
int addrconf_add_linklocal(struct netif_port *dev,
			    const struct in6_addr *addr)
{
        int err=0;
        struct inet_ifaddr *ifa;		
        ...
        err=inet_addr_add(af, dev, &ifa->addr, plen,
                  &ifa->bcast, valid_lft, prefered_lft, scope, flags);
...
/* add ipv6 linklocal address after port start */
int addrconf_addr_gen(struct netif_port *dev)
{
    int err=0;
	...
	if (!in6_dev) {
		...
		RTE_LOG(DEBUG, IFA, "addrconf_addr_gen: device %s not configured\n",dev->name);
	...
	ipv6_addr_set(&linklocal_addr, htonl(0xFE800000), 0, 0, 0);

    char src[64];
    inet_ntop(AF_INET6, (union inet_addr*)&(linklocal_addr), src, sizeof(src));

    addrconf_ifid_eui48(linklocal_addr.s6_addr + 8, dev);
	
                
    char src2[64];
    inet_ntop(AF_INET6, (union inet_addr*)&(linklocal_addr), src2, sizeof(src2));

    err=addrconf_add_linklocal(dev, &linklocal_addr);
...
static int inet_ifaddr_dad_completed(void *arg)
{
    ...
    if(!send_ra)
    {
    ndisc_send_rs(param.dev, NULL, &in6addr_linklocal_allrouters, &linklocal_addr);
    }
...
static void inet_ifaddr_dad_start(struct inet_ifaddr *ifa)
{
    ...
    dpvs_time_rand_delay(&tv, 50000000);
